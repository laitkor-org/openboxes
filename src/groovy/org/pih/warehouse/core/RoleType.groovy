...
enum RoleType {
    ...
    // SuperAdmin: Access to all resources across all tenants
    ROLE_BDSUPERADMIN('SuperAdmin', 0), // ROLE_SUPERUSER

    // TenantAdmin: can do everything a manager can do, plus can rollback shipments, do data imports, and create locations, products, users, stock lists, and people
    ROLE_BDT_ADMIN('Bdt_Admin', 1), // ROLE_ADMIN('Admin', 1),
    ...
    // Openboxes allows the setup administrator to add roles beyond the scope of authentication to enable the admin to assign additional responsibilities and customize access based on the new role types. Once the code is running on Azure, I will take responsibility for customizing these attributes and extended user permissions. Here are the default values:
    // Employee role types
